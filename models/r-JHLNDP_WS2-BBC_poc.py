...
def set_model_time_limit(model):
    ...
    if 'instance_start_time' not in globals() or instance_start_time is None:
...
def solveOptimalLambda_for_pair(i, j, a0, y0, ah, yh):
    ...
    for k in N:
        for m in N:
            if k != m:
                for l in L:
                    x_vars[(k, m, l)] = mdl.addVar(lb=0, vtype=GRB.CONTINUOUS, name=f"x_{i}_{j}_{k}_{m}_{l}")
    mdl.update()
    ...
    for k in N:
        ...
        rhs_expr = (1 - lam) * (a0[(i, k)] - a0[(j, k)]) + lam * (ah[(i, k)] - ah[(j, k)])
        mdl.addConstr(lhs_expr == rhs_expr, name=f"bal_{i}_{j}_{k}")
    for k in N:
        for m in N:
            if k < m:
                for l in L:
...
def runWarmStartPhase():
    ...
    while (UB - LB > tolerance) and (iteration < max_iter):
        ...
        master.optimize()
        if master.status != GRB.OPTIMAL:
            print("Master not optimal after SP#1 cut. Status =", master.status)
            break
        LB = master.ObjVal
        ...
        if unbounded_flag:
            ...
            a0 = {key: (1 - lam_star)*a0[key] + lam_star*aHat[key] for key in a0}
            y0 = {key: (1 - lam_star)*y0[key] + lam_star*yHat[key] for key in y0}
        else:
            UB = min(UB, LB)
            a0 = aHat
            y0 = yHat
...
def callBackFunction(model, where):
    global N, a_vars, y_vars, z_vars, I, D_set, iteration_count, optimal_cut_count, feasibility_cut_count
    if where == GRB.Callback.MIPSOL:
        iteration_count += 1
        print(f"\n--- Phase 2 Callback Iteration {iteration_count} ---")
        ...
        a0 = { (k, k): 0.5 for k in N }
        for i in N:
            for k in N:
                if i != k:
                    a0[(i, k)] = 1.0/(2*n - 2)
        val_y = (n - 2)/(n**2 - n)
        y0 = { (k, m, l): val_y for (k, m, l) in y_vars.keys() }
        
        max_iter_callback = 1  
        for it in range(max_iter_callback):
            print(f"  Callback two-SP iteration {it+1}")
            for (i, j) in D_set:
                mu1, nu1, status1, _, _ = subProblem_ij(i, j, a0, y0)
                if status1 == GRB.OPTIMAL:
                    expr = LinExpr()
                    for k in N:
                        expr += mu1[k] * (model.getVarByName(f"a_{i}_{k}") - model.getVarByName(f"a_{j}_{k}"))
                    for k in N:
                        for m in N:
                            if k < m:
                                for l in L:
                                    expr -= nu1.get((k, m, l), 0) * model.getVarByName(f"y_{k}_{m}_{l}")
                    ...
                    print(f"    SP#1 for ({i},{j}) returned status {status1} - no cut added.")
            for (i, j) in D_set:
                mu2, nu2, status2, _, _ = subProblem_ij(i, j, aHat, yHat)
                if status2 == GRB.OPTIMAL:
                    expr = LinExpr()
                    for k in N:
                        expr += mu2[k] * (model.getVarByName(f"a_{i}_{k}") - model.getVarByName(f"a_{j}_{k}"))
                    for k in N:
                        for m in N:
                            if k < m:
                                for l in L:
                                    expr -= nu2.get((k, m, l), 0) * model.getVarByName(f"y_{k}_{m}_{l}")
                    model.cbLazy(model.getVarByName(f"eta_{i}_{j}") >= expr)
                    optimal_cut_count += 1
                elif status2 in [GRB.UNBOUNDED, GRB.INF_OR_UNBD]:
                    expr = LinExpr()
                    ...
                    print(f"    SP#2 for ({i},{j}) returned status {status2} - no cut added.")
            unbounded_flag = any(subProblem_ij(i, j, aHat, yHat)[2] in [GRB.UNBOUNDED, GRB.INF_OR_UNBD] 
                                  for (i, j) in D_set)
            if unbounded_flag:
                lam_star = computeOptimalLambdaSystem(a0, y0, aHat, yHat)
                print("    Optimal lambda from system:", lam_star)
                ...
                a0 = aHat.copy()
                y0 = yHat.copy()
        print(f"  End of callback two-SP process after {max_iter_callback} iterations.")

def runPhase2Benders():
    master, eta_vars = setupMasterProblemModelPhase2()
    addStoredCutsToModel(master, eta_vars)
    print("Starting Phase 2 MILP with modified callback...")
    start_time = time.time()
    ...
    print("Solution (variables with value 1):")
    for i in N:
        for k in N:
            if a_vars[(i, k)].X > 0.5:
                print(f"a[{i},{k}] = 1")
    for k in N:
        for l in L:
            if z_vars[(k, l)].X > 0.5:
                print(f"z[{k},{l}] = 1")
    for (k, m, l) in y_vars:
        if y_vars[(k, m, l)].X > 0.5:
            print(f"y[{k},{m},{l}] = 1")
    for k in N:
        if I[k].X > 0.5:
            print(f"I[{k}] = 1")
    print(f"Phase 2 done in {total_time:.2f} seconds.")
    print(f"Total optimality cuts added in Phase 2: {optimal_cut_count}")
    print(f"Total feasibility cuts added in Phase 2: {feasibility_cut_count}")
    master.write("final_master_phase2.lp")
    return master
...
def runTwoPhaseBenders():
    ...
    print("Running Phase 1 (Warm-Start with Magnanti–Wong + optimal-lambda)...")
    runWarmStartPhase()  

    print("\nRunning Phase 2 (Integer Model with stored cuts and fixed variables)...")
    runPhase2Benders()
